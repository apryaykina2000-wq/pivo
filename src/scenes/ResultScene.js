...
export default class ResultScene extends Phaser.Scene {
  ...
  create(){
    ...
    // Buttons
    const restart = this.add.image(WIDTH/2 - 130, HEIGHT*0.78, 'ui_btn_restart'.replace('ui_','')).setTexture('btn_restart').setInteractive({useHandCursor:true});
    restart.on('pointerup', ()=> this.scene.start('Game'));

    const home = this.add.image(WIDTH/2 + 130, HEIGHT*0.78, 'btn_home').setInteractive({useHandCursor:true});
