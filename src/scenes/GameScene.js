...
export default class GameScene extends Phaser.Scene {
  ...
  create(){
    ...
    this.groundY = 640; // ниже, чтобы показать больше травы и земли
    this.ground = this.physics.add.staticImage(WIDTH/2, this.groundY+20, 'ground_tile').setVisible(false).setScale(12,1.2);
    this.groundVis = this.add.tileSprite(0, this.groundY, WIDTH, 200, 'ground_tile').setOrigin(0,0.5);

    // Player
    this.player = this.physics.add.sprite(220, this.groundY-60, 'run1').setCollideWorldBounds(true);
    // используем мировую гравитацию; не обнуляем её
    this.player.setScale(2);
    this.player.play('run');
    // подогнать хитбокс под увеличенный спрайт
    ...
    // Timers for spawn
    this.itemTimer = this.time.addEvent({ delay: 1300, loop: true, callback: this.spawnItem, callbackScope: this });
    this.obstacleTimer = this.time.addEvent({ delay: 2000, loop: true, callback: this.spawnObstacle, callbackScope: this });
  }
