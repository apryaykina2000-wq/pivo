...
export default class MenuScene extends Phaser.Scene {
  ...
  create(){
    ...
    const btnText = this.add.text(WIDTH/2, HEIGHT*0.78, 'ИГРАТЬ', { fontFamily:'CenturyGothicBold', fontSize:'36px', color:'#2b1600' }).setOrigin(0.5);
    btn.on('pointerdown', ()=> { btn.setTexture('btn_play_pressed'); btnText.y += 1; });
    btn.on('pointerup', ()=> { btn.setTexture('btn_play'); btnText.y -= 1; this.scene.start('Game'); });
